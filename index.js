...
if (!TOKEN || !ORDER_CHANNEL_ID) {
  console.error("DISCORD_TOKEN och ORDER_CHANNEL_ID måste finnas som miljövariabler.");
  process.exit(1);
}
...
if (!Array.isArray(allowedItems) || allowedItems.length === 0) {
  console.error("items.json saknas, är tom eller innehåller fel format.");
  process.exit(1);
}
...
function buildSummary() {
  ...
  lines.push("_Skriv ett föremålsnamn för 1 st, eller t.ex. `Bhorgolpäls 3`._");
  lines.push("_Skriv `-Bhorgolpäls` för att ta bort hela din egen beställning._");
...
client.on("messageCreate", async message => {
  try {
    ...
    if (content.startsWith("-")) {
      ...
      if (canonicalItem && data.orders[canonicalItem]?.[message.author.id]) {
        delete data.orders[canonicalItem][message.author.id];
        if (Object.keys(data.orders[canonicalItem]).length === 0) delete data.orders[canonicalItem];
        saveData();
      }
      await message.delete().catch(() => {});
      await updateSummary(message.channel);
